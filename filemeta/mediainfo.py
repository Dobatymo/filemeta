...
class MediaInfoFields:
	...
	def mediainfo(self, path: str, logged_keys: Optional[Dict[str, Set[Tuple[str, str]]]]) -> Iterator[Tuple[Tuple[str, str], Any]]:
					...
					if field_type:
						try:
							...
							if field_type in widen_type_to:
								...
								logger.warning("Converting %s/%s: `%s` to %s failed: %s. Mapping to %s instead.", t.track_type, k, v, field_type, e, new_type)
								field_type = new_type
							elif field_type == "string":
								raise ValueError(f"Converting {t.track_type}/{k}: `{v[:10]}` to {field_type} failed: {e}")
							else:
